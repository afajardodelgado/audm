...
Auth is currently **dormant**. Every document, highlight, and comment is attributed to a single synthetic `local` user, and all routes are public. The Supabase client/server stubs (`src/lib/supabase/`), the pass-through `src/middleware.ts`, and the `User` table exist so that switching auth on later is a contained change — the `User.id` is designed to mirror a Supabase auth uid. Until then, treat the app as single-user.
