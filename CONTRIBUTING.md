...
> **Lint scope:** `src/generated/**` (the machine-written Prisma client) is excluded in `eslint.config.mjs`, so `npm run lint` reports only real findings. The baseline is clean — a change should leave it that way.
...
- **Next.js 16 specifics:** this is a newer Next than most references assume. Route handler `params` are a `Promise` (`await params`). The middleware file convention is renamed — the request hook lives in `src/proxy.ts` (exporting `proxy`), not `middleware.ts`. The reader page is `force-dynamic`. PDF/EPUB/OCR/canvas packages are listed in `serverExternalPackages` so their native/worker deps resolve at runtime — add new native packages there too. See `node_modules/next/dist/docs/` for the in-tree guides (per `AGENTS.md`).
